...
interface OverseerrPayload {
    ...
    message?: string;
    image?: string;
    email?: string; // We will pass user email in the payload
    notifyuser_email?: string;
    requestedBy_email?: string;
}

export const sendFcmNotification = functions.https.onRequest(async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).send("Method Not Allowed");
    return;
  }

  const payload = req.body as OverseerrPayload;
